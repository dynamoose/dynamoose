...
Condition.prototype.requestObject = function() {
	...
	let index = 0;
	function main(input, settings = {"parenthesis": false}) {
		return input.reduce((object, entry) => {
			let expression = "";
			if (Array.isArray(entry[0])) {
				const result = main(entry, {"parenthesis": true});
				...
				expression = `(${result.ConditionExpression})`;
			...
			object.ConditionExpression = `${object.ConditionExpression}${object.ConditionExpression !== "" ? " AND " : ""}${expression}`;
