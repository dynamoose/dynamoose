...
Scan.prototype.exec = function (next) {
  ...
  const result = new Promise((resolve,reject) => {
    ...
    async function scanByRawFilter() {
            ...
            try {
              ...
              if (data.Items !== undefined) {
                ...
                if(options.one) {
                  ...
                  return resolve(models.filter(item => !(schema.expires && schema.expires.returnExpiredItems === false && item[schema.expires.attribute] && item[schema.expires.attribute] < new Date()))[0]);
                }
    ...
    if(Model$.options.waitForActive) {
      const result = Model$.table.waitForActive()
        .then(scan)
        .catch(reject)
      resolve(result)
    }
