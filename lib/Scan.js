...
Scan.prototype.exec = async function (next) {
  ...
  function scanSegment (segment) {
    ...
    async function scanOne () {
      ...
      const shouldContinue = await theScan.Model._emit('model:scan', 'request:pre', {'event': {'scan': theScan, 'callback': next, 'scanReq': scanOneReq}, 'actions': {'updateScanReq' (req) { scanOneReq = req; }}}, deferredMain);
      if (shouldContinue === false) { return; }
      theScan.Model.$__.base.ddb().scan(scanOneReq, async (err, data) => {
        const shouldContinue = await Model._emit('model:scan', 'request:post', {'event': {'scan': theScan, 'callback': next, data, 'error': err}, 'actions': {'updateError' (error) { err = error; }, 'updateData' (myData) { data = myData; }}}, deferredMain);
        if (shouldContinue === false) { return; }
        if (err) {
