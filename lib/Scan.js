  ...
  function scanSegment (segment) {
    ...
    async function scanOne () {
      ...
      that.Model.$__.base.ddb().scan(scanOneReq, async (err, data) => {
        const shouldContinue = await Model._emit('model:scan', 'request:post', {'event': {'scan': that, 'callback': next, data, 'error': err}, 'actions': {'updateError' (error) { err = error; }, 'updateData' (myData) { data = myData; }}}, deferredMain);
        if (shouldContinue === false) { return; }
        if (err) {
