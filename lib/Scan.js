...
Scan.prototype.exec = function (next) {
  ...
  var theScan = this;
  Model._emit('model:scan', 'exec:start', {event: {scan: this, callback: next}, actions: {updateCallback: function(fn) {next = fn;}}});
  ...
  function scanSegment (segment) {
    ...
    scanOne();
    function scanOne() {
      Model._emit('model:scan', 'request:pre', {event: {scan: theScan, callback: next, scanReq: scanOneReq}, actions: {updateScanReq: function(req) {scanOneReq = req;}}});
      debug('scan request', scanOneReq);
      Model.$__.base.ddb().scan(scanOneReq, function(err, data) {
        Model._emit('model:scan', 'request:post', {event: {scan: theScan, callback: next, data: data, error: err}, actions: {updateError: function(error) {err = error;}, updateData: function(myData) {data = myData;}}});
        if(err) {
