...
Scan.prototype.exec = function (next) {
  ...
  function scanSegment (segment) {
    ...
    async function scanOne() {
      ...
      let shouldContinue = await theScan.Model._emit('model:scan', 'request:pre', {event: {scan: theScan, callback: next, scanReq: scanOneReq}, actions: {updateScanReq: function(req) {scanOneReq = req;}}}, deferred);
      if (shouldContinue === false) { return; }
      theScan.Model.$__.base.ddb().scan(scanOneReq, async function(err, data) {
        let shouldContinue = await Model._emit('model:scan', 'request:post', {event: {scan: theScan, callback: next, data: data, error: err}, actions: {updateError: function(error) {err = error;}, updateData: function(myData) {data = myData;}}});
        if (shouldContinue === false) { return; }
        if(err) {
