...
Scan.prototype.exec = async function (next) {
  ...
  function scanSegment (segment) {
    ...
    async function scanOne () {
      ...
      const shouldContinue = await that.Model._emit('model:scan', 'request:pre', {'event': {'scan': that, 'callback': next, 'scanReq': scanOneReq}, 'actions': {'updateScanReq' (req) { scanOneReq = req; }}}, deferredMain);
      if (shouldContinue === false) { return; }
      that.Model.$__.base.ddb().scan(scanOneReq, async (err, data) => {
        const shouldContinueB = await Model._emit('model:scan', 'request:post', {'event': {'scan': that, 'callback': next, data, 'error': err}, 'actions': {'updateError' (error) { err = error; }, 'updateData' (myData) { data = myData; }}}, deferredMain);
