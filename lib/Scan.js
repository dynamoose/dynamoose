...
function Scan (Model, filter, options) {
  ...
  if (typeof filter === 'string') {
    ...
    if (options && options.useRawAwsFilter) {
      // if use raw, just assign the filter
      this.filters = filter;
    } else {
...
Scan.prototype.exec = function (next) {
  ...
  if (options.useRawAwsFilter) {
    ...
    // TODO validation of the custom raw filter
    scanReq = this.filters;

    // use the document client in aws-sdk
    var AWS = require('dynamoose').AWS;
    ...
    // use the default dynamodb client
    dbClient = Model.$__.base.ddb();
  }
  ...
  function scan () {
    ...
    function scanOne() {
      ...
      // Model.$__.base.ddb()
      // use previous defined db client
      dbClient
      .scan(scanReq, function(err, data) {
        if(err) {
