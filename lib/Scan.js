...
Scan.prototype.exec = function (next) {
  ...
  function scanSegment (segment) {
    ...
    scanOne();
    function scanOne() {
      theScan.Model._emit('model:scan', 'request:pre', {event: {scan: theScan, callback: next, scanReq: scanOneReq}, actions: {updateScanReq: function(req) {scanOneReq = req;}}});
      debug('scan request', scanOneReq);
      theScan.Model.$__.base.ddb().scan(scanOneReq, function(err, data) {
