...
Query.prototype.exec = async function (next) {
  ...
  var theQuery = this;
  this.Model._emit('model:query', 'exec:start', {event: {query: this, callback: next}, actions: {updateCallback: function(fn) {next = fn;}}});
  ...
  function query () {
    ...
    async function queryOne() {
      ...
      let shouldContinue = await theQuery.Model._emit('model:query', 'request:pre', {event: {query: theQuery, callback: next, queryReq: queryReq}, actions: {updateQueryReq: function(req) {queryReq = req;}}}, deferred);
      if (shouldContinue === false) { return; }
      Model$.base.ddb().query(queryReq, async function(err, data) {
        let shouldContinue = await theQuery.Model._emit('model:query', 'request:post', {event: {query: theQuery, callback: next, data: data, error: err}, actions: {updateError: function(error) {err = error;}, updateData: function(myData) {data = myData;}}}, deferred);
        if (shouldContinue === false) { return; }
        if(err) {
