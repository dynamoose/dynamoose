  ...
  function query () {
    ...
    function queryOne() {
      ...
      Model$.base.ddb().query(queryReq, function(err, data) {
        theQuery.Model._emit('model:scan', 'request:post', {event: {query: theQuery, callback: next, data: data, error: err}, actions: {updateError: function(error) {err = error;}, updateData: function(myData) {data = myData;}}});
        if(err) {
