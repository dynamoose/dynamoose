  ...
  function query () {
    ...
    async function queryOne () {
      ...
      Model$.base.ddb().query(queryReq, async (err, data) => {
        const shouldContinueB = await that.Model._emit('model:query', 'request:post', {'event': {'query': that, 'callback': next, data, 'error': err}, 'actions': {'updateError' (error) { err = error; }, 'updateData' (myData) { data = myData; }}}, deferred);
