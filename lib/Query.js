  ...
  function query () {
    ...
    async function queryOne() {
      ...
      Model$.base.ddb().query(queryReq, async function(err, data) {
        let shouldContinue = await theQuery.Model._emit('model:query', 'request:post', {event: {query: theQuery, callback: next, data: data, error: err}, actions: {updateError: function(error) {err = error;}, updateData: function(myData) {data = myData;}}});
        if (shouldContinue === false) { return; }
        if(err) {
