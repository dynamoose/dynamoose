...
Query.prototype.exec = async function (next) {
  ...
  if (this.query.rangeKey) {
    const rangeKey = this.query.rangeKey;
  ...
  function query () {
    ...
    async function queryOne () {
      ...
      const shouldContinue = await theQuery.Model._emit('model:query', 'request:pre', {'event': {'query': theQuery, 'callback': next, queryReq}, 'actions': {'updateQueryReq' (req) { queryReq = req; }}}, deferred);
      if (shouldContinue === false) { return; }
      Model$.base.ddb().query(queryReq, async (err, data) => {
        const shouldContinue = await theQuery.Model._emit('model:query', 'request:post', {'event': {'query': theQuery, 'callback': next, data, 'error': err}, 'actions': {'updateError' (error) { err = error; }, 'updateData' (myData) { data = myData; }}}, deferred);
        if (shouldContinue === false) { return; }
        if (err) {
