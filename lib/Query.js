...
Query.prototype.exec = function (next) {
  ...
  function query () {
    ...
    function queryOne() {
      theQuery.Model._emit('model:query', 'request:pre', {event: {query: theQuery, callback: next, queryReq: queryReq}, actions: {updateQueryReq: function(req) {queryReq = req;}}});
      debug('DynamoDB Query: %j', queryReq);
      Model$.base.ddb().query(queryReq, function(err, data) {
        theQuery.Model._emit('model:query', 'request:post', {event: {query: theQuery, callback: next, data: data, error: err}, actions: {updateError: function(error) {err = error;}, updateData: function(myData) {data = myData;}}});
        if(err) {
