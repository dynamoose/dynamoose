...
Query.prototype.exec = function (next) {
  ...
  var theQuery = this;
  Model._emit('model:query', 'exec:start', {event: {query: this, callback: next}, actions: {updateCallback: function(fn) {next = fn;}}});
  ...
  function query () {
    ...
    function queryOne() {
      Model._emit('model:query', 'request:pre', {event: {query: theQuery, callback: next, queryReq: queryReq}, actions: {updateQueryReq: function(req) {queryReq = req;}}});
      debug('DynamoDB Query: %j', queryReq);
      Model$.base.ddb().query(queryReq, function(err, data) {
        Model._emit('model:scan', 'request:post', {event: {query: theQuery, callback: next, data: data, error: err}, actions: {updateError: function(error) {err = error;}, updateData: function(myData) {data = myData;}}});
        if(err) {
