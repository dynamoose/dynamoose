...
async function getTableDetails(model, forceRefresh = false) {
	if (forceRefresh || !model.latestTableDetails) {
		...
		model.latestTableDetails = tableDetails;
	}
...
Model.prototype.update = function(keyObj, updateObj, callback) {
	...
	const getUpdateExpressionObject = () => {
		const updateTypes = [
			{"name": "$SET", "operator": " = "},
			{"name": "$ADD"},
		...
		let index = 0;
		const returnObject = Object.keys(updateObj).reduce((accumulator, key) => {
			...
			Object.keys(value).forEach((subKey) => {
				...
				if (this.schema.getAttributeTypeDetails(subKey).dynamodbType === "L") {
					accumulator.ExpressionAttributeValues[expressionValue] = Array.isArray(accumulator.ExpressionAttributeValues[expressionValue]) ? accumulator.ExpressionAttributeValues[expressionValue] : [accumulator.ExpressionAttributeValues[expressionValue]];
			...
			return accumulator;
		}, {"ExpressionAttributeNames": {}, "ExpressionAttributeValues": {}, "UpdateExpression": updateTypes.map((a) => a.name).reduce((accumulator, key) => {
	...
	const promise = this.pendingTaskPromise().then(() => aws.ddb().updateItem({
		"Key": this.Document.toDynamo(keyObj),
		"ReturnValues": "ALL_NEW",
		...getUpdateExpressionObject(),
		"TableName": this.name
	}).promise());
