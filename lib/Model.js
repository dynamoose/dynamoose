...
Model.prototype.batchGet = function (keys, settings = {}, callback) {
	...
	const prepareResponse = async (response) => {
		...
		return keyObjects.reduce((result, key) => {
			...
			if (item) {
				result.push(item);
			} else if (response.UnprocessedKeys[this.name]) {
				item = response.UnprocessedKeys[this.name].Keys.find((item) => keyProperties.every((keyProperty) => item[keyProperty] === key[keyProperty]));
				if (item) {
