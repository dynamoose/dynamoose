...
/**
 * creates, configures, and returns a new model
 * @param  {string} name    the name of the new model
 * @param  {object} schema  a built schema class object
 * @param  {object} options optional overrides for a model instance
 * @param  {class}  base    the dynamo base class and config
 * @return {class}          a newly built and initialized model
 */
...
Model.prototype.put = async function(options, next) {
  ...
  try {
    ...
    let tempOverrideOptions = {...this.$__.options};
    let optionsCache = {...schema.options};
    // This is here in the event a model is called with options that should have been in the schema
    if (tempOverrideOptions) {
      schema.addAndParseOptions(tempOverrideOptions, true);
    }
    item = {
    ...
    debug('putItem', item);
    // Here we reset the schema to its default option set
    if (tempOverrideOptions) {
      schema.options = optionsCache;
    }
    model = this;
    model$ = this.$__;
