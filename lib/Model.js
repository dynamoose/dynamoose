...
Model.prototype.update = function (keyObj, updateObj, settings = {}, callback) {
	...
	const getUpdateExpressionObject = async () => {
		...
		Object.values(returnObject.ExpressionAttributeNames).map((attribute, index) => {
			...
			const attributeType = Schema.attributeTypes.find((a) => a.dynamodbType === dynamoType);
	...
	const updateItemParamsPromise = this.pendingTaskPromise().then(async () => ({
		...
		...utils.merge_objects.main({"combineMethod": "object_combine"})((settings.condition ? settings.condition.requestObject() : {}), await getUpdateExpressionObject()),
		"TableName": this.name
	}));
	if (settings.return === "request") {
		if (callback) {
...
Model.prototype.methods = {
	"set": function (name, fn) {
		if (!this.Document[name] || (this.Document[name][Internal.internalProperties] && this.Document[name][Internal.internalProperties].type === "customMethod")) {
			this.Document[name] = (cb) => {
				if (cb) {
					const result = fn.bind(this.Document)(cb);
					if (result instanceof Promise) {
						result.then((result) => cb(null, result)).catch((err) => cb(err));
					}
				} else {
					return new Promise((resolve, reject) => {
						const result = fn.bind(this.Document)((err, result) => {
							if (err) {
								reject(err);
							} else {
								resolve(result);
			...
			this.Document[name][Internal.internalProperties] = {"type": "customMethod"};
		}
	},
	...
	"document": {
		"set": function (name, fn) {
			if (!this.Document.prototype[name] || (this.Document.prototype[name][Internal.internalProperties] && this.Document.prototype[name][Internal.internalProperties].type === "customMethod")) {
				this.Document.prototype[name] = function (cb) {
					if (cb) {
						const result = fn.bind(this)(cb);
						if (result instanceof Promise) {
							result.then((result) => cb(null, result)).catch((err) => cb(err));
						}
					} else {
						return new Promise((resolve, reject) => {
							const result = fn.bind(this)((err, result) => {
								if (err) {
									reject(err);
								} else {
									resolve(result);
				...
				this.Document.prototype[name][Internal.internalProperties] = {"type": "customMethod"};
			}
		},
		"delete": function (name) {
			if (this.Document.prototype[name] && this.Document.prototype[name][Internal.internalProperties] && this.Document.prototype[name][Internal.internalProperties].type === "customMethod") {
				this.Document.prototype[name] = undefined;
			}
		}
	}
};
