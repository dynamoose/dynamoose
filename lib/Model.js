...
Model.prototype.put = async function (options, next) {
  ...
  async function putItem () {
    const shouldContinue = await self.$__.NewModel._emit('model:put', 'request:pre', {'event': {options, item}, 'actions': {'updateItem' (newItem) { item = newItem; }}}, deferred);
    if (shouldContinue === false) { return; }
    model$.base.ddb().putItem(item, async (err) => {
      const shouldContinue = await self.$__.NewModel._emit('model:put', 'request:post', {'event': {options, item, 'error': err}, 'actions': {'updateError' (newErr) { err = newErr; }}}, deferred);
      if (shouldContinue === false) { return; }
      if (err) {
        deferred.reject(err);
      }
...
Model.get = async function (NewModel, key, options, next) {
  ...
  async function get () {
    ...
    const shouldContinue = await NewModel._emit('model:get', 'request:pre', {'event': {'callback': next, key, options, 'getRequest': getReq}, 'actions': {'updateCallback' (cb) { next = cb; }, 'updateKey' (newKey) { key = newKey; }, 'updateOptions' (newOptions) { options = newOptions; }, 'updateGetRequest' (newReq) { getReq = newReq; }}}, deferred);
    if (shouldContinue === false) { return; }
    newModel$.base.ddb().getItem(getReq, async (err, data) => {
      const shouldContinue = await NewModel._emit('model:get', 'request:post', {'event': {'callback': next, key, options, 'error': err, data}, 'actions': {'updateCallback' (cb) { next = cb; }, 'updateKey' (newKey) { key = newKey; }, 'updateOptions' (newOptions) { options = newOptions; }, 'updateError' (newErr) { err = newErr; }, 'updateData' (newData) { data = newData; }}}, deferred);
      if (shouldContinue === false) { return; }
      if (err) {
...
Model.prototype.delete = async function (options, next) {
  ...
  function deleteItem () {
    ...
    model$.base.ddb().deleteItem(getDelete, async (err, data) => {
      if (err) {
