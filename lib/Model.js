...
Model.batchPut = function(NewModel, items, options, next) {
  ...
  batchPutReq.RequestItems[NewModel.$__.name] = putRequests;

  function batchPutItems () {
    debug('batchPutItems', batchPutReq);

    newModel$.base.ddb().batchWriteItem(batchPutReq, function(err, data) {
      if(err) {
        debug('Error returned by batchPutItems', err);
        return deferred.reject(err);
      }
  ...
  if(newModel$.options.waitForActive) {
    newModel$.table.waitForActive().then(batchPutItems);
  } else {
    batchPutItems();
  }
  return deferred.promise.nodeify(next);
...
Model.batchDelete = function(NewModel, keys, options, next) {
  ...
  for (var i=0 ; i<len ; ++i) {
    ...
    if(!key[schema.hashKey.name]) {
      var keyVal = key;
      key = {};
      key[schema.hashKey.name] = keyVal;
    }

    if(schema.rangeKey && !key[schema.rangeKey.name]) {
      deferred.reject(new errors.ModelError('Range key required for hash key \'%s\': %s', key[schema.hashKey.name], schema.hashKey.name));
      return deferred.promise.nodeify(next);
    }
  ...
  var delRequests = keys.map(function(key) {
    var key_element = {};
    key_element[schema.hashKey.name] = schema.hashKey.toDynamo(key[schema.hashKey.name]);
  ...
  batchDelReq.RequestItems[NewModel.$__.name] = delRequests;

  function batchDelItems () {
    debug('batchDelItems', batchDelReq);

    newModel$.base.ddb().batchWriteItem(batchDelReq, function(err, data) {
      if(err) {
        debug('Error returned by batchDelItems', err);
        return deferred.reject(err);
      }
  ...
  if(newModel$.options.waitForActive) {
    newModel$.table.waitForActive().then(batchDelItems);
  } else {
    batchDelItems();
  }
  return deferred.promise.nodeify(next);
