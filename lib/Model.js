	...
	const getUpdateExpressionObject = async () => {
		...
		const returnObject = await Object.keys(updateObj).reduce(async (accumulatorPromise, key) => {
			...
			await Promise.all(Object.keys(value).map(async (subKey) => {
				...
				if (!updateType.attributeOnly) {
					await this.Document.objectFromSchema({[subKey]: dynamoType === "L" && !Array.isArray(subValue) ? [subValue] : subValue}, {"type": "toDynamo"});
				}
			...
			return accumulator;
		}, Promise.resolve({"ExpressionAttributeNames": {}, "ExpressionAttributeValues": {}, "UpdateExpression": updateTypes.map((a) => a.name).reduce((accumulator, key) => {
