...
Model.compile = function compile (name, schema, options, base) {
  ...
  // values are reused.
  var nameDescriptor = Object.getOwnPropertyDescriptor(NewModel, 'name');
  ...
  NewModel._emit = function(type, stage, obj) {
    ...
    this.$__.plugins.forEach(function(plugin) {
...
Model.prototype.put = function(options, next) {
  var self = this;
  this.$__.NewModel._emit('model:put', 'put:called', {event: {callback: next, options: options}, actions: {updateCallback: function(cb) {next = cb;}, updateOptions: function(newOptions) {options = newOptions;}}});
  debug('put', this);
  var deferred = Q.defer();

  function putItem() {
    self.$__.NewModel._emit('model:put', 'request:pre', {event: {options: options, item: item}, actions: {updateItem: function(newItem) {item = newItem;}}});
    model$.base.ddb().putItem(item, function(err) {
      self.$__.NewModel._emit('model:put', 'request:post', {event: {options: options, item: item, error: err}, actions: {updateError: function(newErr) {err = newErr;}}});
      if(err) {
        deferred.reject(err);
      }
...
Model.get = function(NewModel, key, options, next) {
  NewModel._emit('model:get', 'get:called', {event: {callback: next, key: key, options: options}, actions: {updateCallback: function(cb) {next = cb;}, updateKey: function(newKey) {key = newKey;}, updateOptions: function(newOptions) {options = newOptions;} }});
  debug('Get %j', key);
  var deferred = Q.defer();
  ...
  function get () {
    NewModel._emit('model:get', 'request:pre', {event: {callback: next, key: key, options: options, getRequest: getReq}, actions: {updateCallback: function(cb) {next = cb;}, updateKey: function(newKey) {key = newKey;}, updateOptions: function(newOptions) {options = newOptions;}, updateGetRequest: function(newReq) {getReq = newReq;} }});
    debug('getItem', getReq);
    newModel$.base.ddb().getItem(getReq, function(err, data) {
      NewModel._emit('model:get', 'request:post', {event: {callback: next, key: key, options: options, error: err, data: data}, actions: {updateCallback: function(cb) {next = cb;}, updateKey: function(newKey) {key = newKey;}, updateOptions: function(newOptions) {options = newOptions;}, updateError: function(newErr) {err = newErr;}, updateData: function(newData) {data = newData;} }});
      if(err) {
