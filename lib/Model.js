...
//   $ADD: {count: 1}
// });
// NewModel.update({id: 123}, { $SET: {a: 1, b: 2} });
// NewModel.update({id: 123}, {a: 1, b: 2} ); // Defaults to set (same as above)*/
Model.update = async function (NewModel, key, update, options, next) {
  ...
  function Operations () {
    ...
    this.addDelete = function (name, item) {
      if (schema.hashKey.name !== name && (schema.rangeKey || {}).name !== name) {
        this.DELETE[name] = item;
      }
    };
    ...
    this.getUpdateExpression = function (getUpdateReq) {
      ...
      for (name in this.DELETE) {
        item = this.DELETE[name];
        ...
        deleteExpressions.push(`${attrName} ${valName}`);
  ...
  if (update.$SET || !update.$SET && !update.$REMOVE && !update.$ADD && !update.$DELETE) {
    const updateSET = update.$SET || update;

    for (const setItem in updateSET) {
      const setAttr = schema.attributes[setItem];
      if (setAttr || schema.options.saveUnknown) {
        const val = updateSET[setItem];
        ...
        if (removeParams) {
          operations.addRemove(setItem, null);
        } else {
          try {
            if (setAttr) {
              operations.addSet(setItem, await setAttr.toDynamo(val));
            } else if (schema.options.saveUnknown) {
              operations.addSet(setItem, await Attribute.create(schema, setItem, val).toDynamo(val));
            }
          } catch (err) {
            deferred.reject(err);
            return deferred.promise.nodeify(next);
  ...
  if (update.$DELETE) {
    for (const deleteItem in update.$DELETE) {
      const deleteAttr = schema.attributes[deleteItem];
      const deleteVal = update.$DELETE[deleteItem];
      try {
        if (deleteAttr === 'list') {
          const err = new Error('$DELETE does not support type list');
          deferred.reject(err);
          return deferred.promise.nodeify(next);
        }

        operations.addDelete(deleteItem, await deleteAttr.toDynamo(deleteVal));
      } catch (err) {
        deferred.reject(err);
        return deferred.promise.nodeify(next);
      }
