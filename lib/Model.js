...
Model.prototype.update = function(keyObj, updateObj, callback) {
	...
	const getUpdateExpressionObject = async () => {
		const updateTypes = [
			{"name": "$SET", "operator": " = ", "objectFromSchemaSettings": {"validate": true, "enum": true}},
			{"name": "$ADD"},
		...
		const returnObject = await Object.keys(updateObj).reduce(async (accumulatorPromise, key) => {
			...
			await Promise.all(Object.keys(value).map(async (subKey) => {
				...
				if ((updateType.objectFromSchemaSettings || {}).defaults || (updateType.objectFromSchemaSettings || {}).forceDefault) {
					const defaultValue = await this.schema.defaultCheck(subKey, /*updateType.attributeOnly ? */undefined/* : subValue*/, updateType.objectFromSchemaSettings);  // TODO: uncomment out the sections once we add unit tests for this case
					if (defaultValue) {
				...
				if ((updateType.objectFromSchemaSettings || {}).required) {
					await this.schema.requiredCheck(subKey, /*updateType.attributeOnly ? */undefined/* : subValue*/); // TODO: uncomment out the sections once we add unit tests for this case
				}
