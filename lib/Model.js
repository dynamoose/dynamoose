...
Model.prototype.get = function(key, callback) {
	const documentify = (document) => (new this.Document(document)).conformToSchema();
	...
	if (callback) {
		promise.then((response) => callback(null, response.Item ? documentify(response.Item) : undefined)).catch((error) => callback(error));
	} else {
		return (async () => {
			const response = await promise;
			return response.Item ? documentify(response.Item) : undefined;
		})();
	}
};

Model.prototype.scan = {"carrier": require("./Scan")};
