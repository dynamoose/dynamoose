...
Model.prototype.put = async function (options, next) {
  debug('put', this);
  const deferred = Q.defer();
  const shouldContinue = await this.$__.NewModel._emit('model:put', 'put:called', {'event': {'callback': next, options}, 'actions': {'updateCallback' (cb) { next = cb; }, 'updateOptions' (newOptions) { options = newOptions; }}}, deferred);
  if (shouldContinue === false) { return; }
  let item;

  const putItem = async () => {
    const shouldContinueB = await this.$__.NewModel._emit('model:put', 'request:pre', {'event': {options, item}, 'actions': {'updateItem' (newItem) { item = newItem; }}}, deferred);
    if (shouldContinueB === false) { return; }
    this.$__.base.ddb().putItem(item, async (err) => {
      const shouldContinueC = await this.$__.NewModel._emit('model:put', 'request:post', {'event': {options, item, 'error': err}, 'actions': {'updateError' (newErr) { err = newErr; }}}, deferred);
      if (shouldContinueC === false) { return; }
      if (err) {
        deferred.reject(err);
      }
...
Model.get = async function (NewModel, key, options, next) {
  debug('Get %j', key);
  const deferred = Q.defer();
  const shouldContinue = await NewModel._emit('model:get', 'get:called', {'event': {'callback': next, key, options}, 'actions': {'updateCallback' (cb) { next = cb; }, 'updateKey' (newKey) { key = newKey; }, 'updateOptions' (newOptions) { options = newOptions; }}}, deferred);
  if (shouldContinue === false) { return; }
  ...
  async function get () {
    ...
    const shouldContinueB = await NewModel._emit('model:get', 'request:pre', {'event': {'callback': next, key, options, 'getRequest': getReq}, 'actions': {'updateCallback' (cb) { next = cb; }, 'updateKey' (newKey) { key = newKey; }, 'updateOptions' (newOptions) { options = newOptions; }, 'updateGetRequest' (newReq) { getReq = newReq; }}}, deferred);
    if (shouldContinueB === false) { return; }
    newModel$.base.ddb().getItem(getReq, async (err, data) => {

      const emitEventObject = {'callback': next, key, options, 'error': err, data};
      const emitObject = {'event': emitEventObject, 'actions': {'updateCallback' (cb) { next = cb; }, 'updateKey' (newKey) { key = newKey; }, 'updateOptions' (newOptions) { options = newOptions; }, 'updateError' (newErr) { err = newErr; }, 'updateData' (newData) { data = newData; }}};
      const shouldContinueC = await NewModel._emit('model:get', 'request:post', emitObject, deferred);
      if (shouldContinueC === false) { return; }
