...
Model.update = async function (NewModel, key, update, options, next) {
  ...
  function Operations () {
    ...
    this.getUpdateExpression = function (getUpdateReq) {
      ...
      if (deleteExpressions.length > 0) {
        updateExpression += `DELETE ${addExpressions.join(',')} `;
      }
  ...
  if (update.$PUT || !update.$PUT && !update.$REMOVE && !update.$ADD && !update.$DELETE) {
  ...
  if (update.$DELETE) {
    for (const deleteItem in update.$DELETE) {
      ...
      try {
        if (deleteAttr) {
          if (deleteAttr.type.name !== 'list') {
            operations.addDelete(deleteItem, await deleteAttr.toDynamo(deleteVal));
          }
          // else {
          //   operations.addListAppend(deleteItem, await deleteAttr.toDynamo(deleteVal));
          // }
        } else if (schema.options.saveUnknown) {
          operations.addDelete(deleteItem, await Attribute.create(schema, deleteItem, deleteVal).toDynamo(deleteVal));
        }
      } catch (err) {
        deferred.reject(err);
        return deferred.promise.nodeify(next);
