...
class Model {
	constructor(name, schema, options = {}) {
		...
		returnObject.transaction = [
			...
			const func = currentValue.function || (this[key] || (() => {})).bind(this); // TODO: fix this. We shouldn't be using a default function. If we remove it tho, there is an issue where `this` is not a reference to the model instance. This only occurs if the `new` keyword is omited. The solution here, is to make a breaking change and switch the Model to be a class vs a function. This will require us updating the documentation and test suite.
...
Model.prototype.get = function(key, settings = {}, callback) {
	...
	const documentify = (document) => (new this.Document(document, {"fromDynamo": true})).conformToSchema({"customTypesDynamo": true, "checkExpiredItem": true, "saveUnknown": true, "type": "fromDynamo"});
