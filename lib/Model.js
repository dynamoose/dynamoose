...
function Model(name, schema, options = {}) {
	...
	// Run setup flow
	const setupFlowPromiseMappedArray = setupFlow.map((flow) => (flow.promise || flow));
	const setupFlowPromise = setupFlowPromiseMappedArray.reduce((existingFlow, flow) => existingFlow.then(() => flow()), Promise.resolve());
	...
	return new Promise((resolve, reject) => {
		...
		async function check() {
			...
			if ((Date.now() - start) >= model.options.waitForActive.check.timeout) {
				return reject(new Error.WaitForActiveTimeout(`Wait for active timed out after ${Date.now() - start}.`));
			} else {
				check();
			}
		}
