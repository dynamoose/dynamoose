...
const customMethodFunctions = (type) => {
	...
	return {
		"set": function (name, fn) {
			const self = this;
			if (!entryPoint(this)[name] || (entryPoint(this)[name][Internal.internalProperties] && entryPoint(this)[name][Internal.internalProperties].type === "customMethod")) {
				entryPoint(this)[name] = function (cb) {
					const bindObject = type === "document" ? this : self.Document;
					if (cb) {
						const result = fn.bind(bindObject)(cb);
						if (result instanceof Promise) {
							result.then((result) => cb(null, result)).catch((err) => cb(err));
						}
					} else {
						return new Promise((resolve, reject) => {
							const result = fn.bind(bindObject)((err, result) => {
								if (err) {
									reject(err);
								} else {
									resolve(result);
				...
				entryPoint(this)[name][Internal.internalProperties] = {"type": "customMethod"};
			}
		},
