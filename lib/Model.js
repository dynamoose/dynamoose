...
Model.prototype.put = async function (options, next) {
  ...
  const putItem = async () => {
    const shouldContinue = await this.$__.NewModel._emit('model:put', 'request:pre', {'event': {options, item}, 'actions': {'updateItem' (newItem) { item = newItem; }}}, deferred);
    if (shouldContinue === false) { return; }
    this.$__.base.ddb().putItem(item, async (err) => {
      const shouldContinue = await this.$__.NewModel._emit('model:put', 'request:post', {'event': {options, item, 'error': err}, 'actions': {'updateError' (newErr) { err = newErr; }}}, deferred);
      if (shouldContinue === false) { return; }
      if (err) {
        deferred.reject(err);
      }
...
Model.get = async function (NewModel, key, options, next) {
  ...
  async function get () {
    ...
    newModel$.base.ddb().getItem(getReq, async (err, data) => {
      ...
      const shouldContinue = await NewModel._emit('model:get', 'request:post', emitObject, deferred);
