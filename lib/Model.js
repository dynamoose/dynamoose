...
Model.prototype.put = async function(options = {}, next) {
  ...
  if(next){
    this.put(options)
      .then( result => {next(null,result)}, err => {next(err) })
    return
  }
...
Model.get = async function(NewModel, key, options, next) {
  ...
  if(next){
    Model.get(NewModel, key, options)
      .then(result => {next(undefined, result)}, err => {next(err)})
    return
  }
...
Model.update = function(NewModel, key, update, options, next) {
  ...
  if(next){
    this.update(NewModel, key, update, options)
      .then(result => {next(undefined,result)},err => {next(err)})
    return
  }
...
Model.delete = async function (NewModel, key, options, next) {
  ...
  if(next){
    Model.delete(NewModel, key, options)
      .then(result => {next(undefined, result)}, err => {next(err)})
    return
  }
  const schema = NewModel.$__.schema;
...
Model.batchGet = async function(NewModel, keys, options, next) {
  ...
  if(next){
    this.batchGet(NewModel, keys, options)
      .then(result => next(undefined, result), err => next(err))
    return;
  }
...
Model.batchPut = async function(NewModel, items, options, next) {
  ...
  if(next){
    this.batchPut(NewModel,items,options)
      .then(result => next(undefined, result), err => next(err))
  }
...
Model.batchDelete = async function(NewModel, keys, options, next) {
  ...
  if(next){
    this.batchDelete(NewModel,keys,options)
      .then(result => next(undefined, result), err => next(err))
    return
  }
