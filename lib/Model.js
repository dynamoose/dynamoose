...
Model.update = function(NewModel, key, update, options, next, emptyarrayallowed) {
  debug('Update %j', key);
  var deferred = Q.defer();
  if(key === null || key === undefined) {
    deferred.reject(new errors.ModelError('Key required to get item'));
  ...
  if(update.$PUT || (!update.$PUT && !update.$DELETE && !update.$ADD)) {
    ...
    for(var putItem in updatePUT) {
      ...
      if(putAttr) {
        ...
        var removeparams;
        if (emptyarrayallowed === undefined || emptyarrayallowed === null || emptyarrayallowed === false) {
          removeparams = val === null || val === undefined || val === '' || (Array.isArray(val) && val.length === 0);
        }
        else {
          removeparams = val === null || val === undefined || val === '';
        }
        if(removeparams) {
