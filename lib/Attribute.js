...
function Attribute(schema, name, value) {
  ...
  //console.log('CREATE: NAME: ' + name + ' VALUE: ' + value);
  if (value.type){
    this.options = value;
  ...
  if ( this.type.name === 'map'){
    ...
    for (var subattrName in value){
      ...
      this.attributes[subattrName] = module.exports.create(this, subattrName, value[subattrName]);
    }
    ...
    for (var i = 0; i < value.length; i++) {
      this.attributes[i] = module.exports.create(this, 0, value[i]);
...
Attribute.prototype.setType = function(value) {
  ...
  //console.log('VALUE: ' + value + ' ' + 'isArray( ' + util.isArray(typeVal) + ')' + ' T ' + typeof typeVal[0] + ' L '+ typeVal.length);
  if (util.isArray(typeVal) && typeVal.length === 1 && typeof typeVal[0] === 'object'){
    ...
    throw new errors.SchemaError('Invalid attribute type: ' + typeVal);
  }
...
Attribute.prototype.toDynamo = function(val, noSet, model) {
  ...
  //console.log('toDynamo: ' + this.type.name + ' val:' + val);
...
Attribute.prototype.parseDynamo = function(json) {
  ...
  //console.log('NAME:' + this.name + ' VAL: ' + val));
