  ...
  if (this.type.name === 'map') {
    if (value.type) {
      if (!value.map) {
        throw new errors.SchemaError(
          `No map schema given for attribute: ${this.name}`
        );
      }
      value = value.map;
    }
    for (const subattrName in value) {
      if (this.attributes[subattrName]) {
        throw new errors.SchemaError(
          `Duplicate attribute: ${subattrName} in ${this.name}`
        );
      }

      this.attributes[subattrName] = module.exports.create(
        schema,
        subattrName,
        value[subattrName]
      );
    ...
    if (value.type) {
      if (!value.list || !value.list[0]) {
        throw new errors.SchemaError(
          `No list schema given for attribute: ${this.name}`
        );
      }
      value = value.list;
    }
        ...
        if (this.options.set.isAsync) {
          this.set = function(val) {
            return new Promise(resolve =>
              this.options.set.set(val, result => resolve(result))
            );
          };
        } else {
      ...
      if (typeof this.options.fromDynamo === 'function') {
        ...
        if (this.options.fromDynamo.isAsync) {
          this.parseDynamoCustom = function(val) {
            return new Promise(resolve =>
              this.options.fromDynamo.fromDynamo(val, result => resolve(result))
            );
          };
        } else {
      ...
      if (typeof this.options.toDynamo === 'function') {
        ...
        if (this.options.toDynamo.isAsync) {
          this.toDynamoCustom = function(val) {
            return new Promise(resolve =>
              this.options.toDynamo.toDynamo(val, result => resolve(result))
            );
          };
        } else {
      ...
      if (typeof this.options.get === 'function') {
        ...
        if (this.options.get.isAsync) {
          this.get = function(val) {
            return new Promise(resolve =>
              this.options.get.get(val, result => resolve(result))
            );
          };
        } else {
...
Attribute.prototype.setTypeFromRawValue = function(value) {
  ...
  if (Array.isArray(typeVal) || typeVal === 'list') {
    ...
    (Array.isArray(typeVal) && typeVal.length === 1) ||
...
Attribute.prototype.toDynamo = async function(val, noSet, model, options) {
  ...
  if (
    ...
    model.$__ &&
    model.$__.schema &&
    model.$__.schema.expires &&
    model.$__.schema.expires.attribute === this.name
  ) {
    isExpires = true;
  }
  ...
  if (isSet) {
    ...
    if (
      type.dynamo !== 'BOOL' &&
      (type.dynamo !== 'B' || !(val instanceof Buffer))
    ) {
      val = val.toString();
    }
    ...
    if (type.dynamo === 'S') {
      if (this.options.enum) {
        if (this.options.enum.indexOf(val) === -1) {
          throw new errors.ValidationError(
            'Value must be one of : ' + JSON.stringify(this.options.enum)
          );
        }
      }
      if (this.options.trim) {
...
Attribute.prototype.parseDynamo = async function(json) {
  ...
  async function dedynamofy(type, isSet, json, transform, attr) {
    const errorHandlingTransform =
      ...
      async function(v) {
        try {
          ...
          e.message += `\nAttribute "${
            attr.name
          }" of type "${type}" has an invalid value of ${JSON.stringify(json)}`;
    ...
    if (isSet) {
      const set = json[type + 'S'] || json

      return await Promise.all(
        set.map(async function(v) {
    ...
    for (const [name, value] of Object.entries(v)) {
      ...
      if (!subAttr && schema.options.errorUnknown) {
        throw new errors.ParseError(
          `Unknown nested attribute ${name} with value: ${JSON.stringify(
            value
          )}`
        );
      }
      if (
        !subAttr &&
        (schema.options.saveUnknown ||
          (Array.isArray(schema.options.saveUnknown) &&
            schema.options.saveUnknown.indexOf(name) >= 0))
      ) {
...
const createUnknownAttributeFromDynamo = (module.exports.createUnknownAttributeFromDynamo = function(
  schema,
  name,
  dynamoAttribute
) {
  debug(
    'createUnknownAttributeFromDynamo: %j : "%s" : %j',
    schema,
    name,
    dynamoAttribute
  );
...
module.exports.create = function(schema, name, obj) {
  ...
  if (options.hashKey && options.rangeKey) {
    throw new errors.SchemaError(
      'Cannot be both hashKey and rangeKey: ' + name
    );
  }
