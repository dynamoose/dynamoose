...
/**
 * This function sets the type of the attribute based on a RAW value. No type defined - assume this is not a type definition and we must grab type directly from value.
 * @inner
 * @param {(object|function|number|boolean|string|array)} value - The RAW value we want to set the type from. (ex. `{ type: [Function: String], required: true, default: 'Mittens' }`, `function String() { [native code] }`, `1`, `false`, `"puddles"`, `[ 'cute', 'fuzzy' ]`, `{ name: 'mittens', age: 1 }`)
 */
Attribute.prototype.setTypeFromRawValue = function(value) {
  let type;
  const typeVal = value.type || value;

  if (typeof typeVal === 'function') {
    // This will be used when we have something like the following:
  ...
  } else if (Array.isArray(typeVal) || typeVal === 'list') {
    type = 'array';
  } else if (typeVal === 'map') {
    type = 'object';
  } else {
  ...
  // We need to set `isSet` to true or false, this will specify that the DynamoDB type should have a trailing `S` character to designate as a set (ex. `SS`, `NS`)
  // We want to set it to true when typeVal is something like: [String] or [Number]
  // Everything else should not be considered a set
  this.isSet = Array.isArray(typeVal) && typeVal.length === 1;

  // We want to use maps and lists instead of objects to take advantage of newer DynamoDB features
  if (type === 'object') {
    type = 'map';
  } else if (type === 'array') {
    type = 'list';
  ...
  // Ensure the type is a valid DynamoDB/Dynamoose type
...
function lookupType(item) {
  const result = AWS.DynamoDB.Converter.output(item);
