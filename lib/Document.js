...
function DocumentCarrier(model) {
	const internalProperties = Symbol();
	...
	Document.objectFromSchema = async function(object, settings = {"type": "toDynamo"}) {
		const returnObject = Object.keys(object).reduce((finalObject, key) => {
			...
			const expectedType = existsInSchema ? utils.attribute_types.find((type) => type.dynamodbType === model.schema.getAttributeType(key)).javascriptType : null;
			const typeMatches = existsInSchema ? valueType === expectedType : null;
