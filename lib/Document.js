...
function DocumentCarrier(model) {
	...
	Document.objectFromSchema = async function(object, settings = {"type": "toDynamo"}) {
		...
		utils.object.entries(returnObject).filter((item) => item[1] !== undefined && item[1] !== dynamooseUndefined).map((item) => {
			const [key, value] = item;
			if (validParents.find((parent) => key.startsWith(parent))) {
				return;
			}
			...
			if (existsInSchema) {
				...
				if (!isValidType) {
					...
					// 		const subTypeDetails = model.schema.getAttributeTypeDetails(`${key}.${index}`);
					// 		const subInternalAttributeType = subTypeDetails.internalAttributeType;
					// 		const subExpectedJSType = subInternalAttributeType.javascriptType;
					// 		const subType = typeof subValue;
					// 		const subIsValidType = (subExpectedJSType && (() => subType === subExpectedJSType)()) || (((subTypeDetails.customType || {}).functions || {}).isOfType || subInternalAttributeType.isOfType || (() => false))(subValue, settings.type);
					// 		if (!subIsValidType) {
					// 			throw new Error.TypeMismatch(`Expected ${key}.${index} to be of type ${subExpectedJSType}, instead found type ${subType}.`);
					// 		} else if (subInternalAttributeType.isSet) {
					// 			validParents.push(`${key}.${index}`);
					// 		}
					// 	}
					// });
					// validParents.push(key);
				...
				if (!settings.saveUnknown || !utils.dynamoose.saveunknown_attribute_allowed_check(model.schema.getSettingValue("saveUnknown"), key)) {
					// TODO: there is a problem here where it deletes the first key first, which has the potienal to leave items left in the array since the indexes change afterwards
					utils.object.delete(returnObject, key);
				}/* else {
