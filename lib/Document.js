	...
	Document.prepareForObjectFromSchema = function(object, settings) {
		if (settings.updateTimestamps) {
			if (model.schema.settings.timestamps && settings.type === "toDynamo") {
				const date = new Date();
				if (model.schema.settings.timestamps.createdAt && !object[internalProperties].storedInDynamo) {
					object[model.schema.settings.timestamps.createdAt] = date;
				}
				if (model.schema.settings.timestamps.updatedAt) {
	...
	Document.objectFromSchema = async function(object, settings = {"type": "toDynamo"}) {
		const returnObject = Object.keys(object).reduce((finalObject, key) => {
			function main(key) {
				...
				const existsInSchema = model.schema.attributes().includes(key);
				const isCustomType = existsInSchema && model.schema.getAttributeTypeDetails(key).customType;
				const valueType = typeof value;
				const customValueType = isCustomType ? typeof model.schema.getAttributeTypeDetails(key).customType.functions[settings.type](value) : null;
				...
				let typeMatches = existsInSchema ? valueType === expectedType : null;
				if (existsInSchema && attributeType.isOfType && attributeType.isOfType(value, settings.type)) {
					typeMatches = true;
				}
				...
				if ((existsInSchema && (typeMatches || (settings.customTypesDynamo && customValueType === expectedType))) || (settings.saveUnknown && saveUnknownAllowed)) {
					if (!existsInSchema && settings.saveUnknown && saveUnknownAllowed) {
						const foundAttributeType = utils.attribute_types.find((type) => type.isOfType && type.isOfType(value, settings.type, {"saveUnknown": true}));
						if (foundAttributeType) {
							attributeType = foundAttributeType;
						}
					}
					const setValue = attributeType && attributeType[settings.type] ? attributeType[settings.type](value) : value;
					if (attributeType && attributeType.nestedType) {
						utils.object.set(finalObject, key, {});
		...
		if (settings.customTypesDynamo) {
			model.schema.attributes().forEach((key) => {
				...
				if (typeDetails.customType) {
					const value = utils.object.get(returnObject, key);
					utils.object.set(returnObject, key, typeDetails.customType.functions[settings.type](value));
				}
			});
		}
