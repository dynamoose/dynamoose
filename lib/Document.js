...
function DocumentCarrier(model) {
	...
	Document.objectFromSchema = async function(object, settings = {"type": "toDynamo"}) {
		...
		const getValueTypeCheckResult = (value, key, options = {}) => {
			...
			const isValidType = (expectedJSType && (() => type === expectedJSType)()) || (((typeDetails.customType || {}).functions || {}).isOfType || internalAttributeType.isOfType)(value, settings.type);
			return {isValidType, internalAttributeType, typeDetails, type};
		};
		const checkTypeFunction = (item) => {
			...
			if (existsInSchema) {
				const {isValidType, internalAttributeType, typeDetails, type} = getValueTypeCheckResult(value, genericKey, {"standardKey": true});
				if (!isValidType) {
					throw new Error.TypeMismatch(`Expected ${key} to be of type ${internalAttributeType.name.toLowerCase()}, instead found type ${type}.`);
			...
			await Promise.all(Document.attributesWithSchema(returnObject).map(async (key) => {
				...
				if (value === dynamooseUndefined) {
					...
					if (!isDefaultValueUndefined) {
						const {isValidType, internalAttributeType, type} = getValueTypeCheckResult(defaultValue, key);
						if (!isValidType) {
							throw new Error.TypeMismatch(`Expected ${key} to be of type ${internalAttributeType.name.toLowerCase()}, instead found type ${type}.`);
						} else {
