...
function DocumentCarrier(model) {
	...
	Document.attributesWithSchema = function(document) {
		...
		if (attributesWithSchemaCache[attributes] && attributesWithSchemaCache[attributes][document]) {
			return attributesWithSchemaCache[attributes][document];
		}
		...
		attributesWithSchemaCache[attributes] = {[document]: result};
		return result;
	};
