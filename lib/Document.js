...
function DocumentCarrier(model) {
	...
	Document.isDynamoObject = (object, recurrsive = false) => {
		...
		function isValid(value) {
			...
			return typeof value === "object" && keys.length === 1 && utils.attribute_types.map((type) => type.dynamodbType).includes(key) && (nestedResult || Object.keys(value[key]).length === 0 || utils.attribute_types.find((a) => a.dynamodbType === key).isSet);
		}
