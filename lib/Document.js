...
function DocumentCarrier(model) {
	...
	Document.objectFromSchema = async function(object, settings = {"type": "toDynamo"}) {
		...
		function mainHandler(finalObject, key, value) {
			if (value === undefined) {
			...
			const customType = existsInSchema && settings.customTypesDynamo && typeDetails.customType;
			...
			if ((existsInSchema && (typeMatches || (typeof customValue === expectedType) || (value === dynamooseUndefined))) || (settings.saveUnknown && saveUnknownAllowed)) {
				...
				if (attributeType && attributeType.nestedType) {
					...
					Object.keys(setValue).forEach((nestedKey) => {
						...
						mainHandler(finalObject, subKey, utils.object.get(object, subKey));
					});
				} else {
					utils.object.set(finalObject, key, setValue);
				}
			} else if (existsInSchema && !typeMatches && !(typeof customValue === expectedType)) {
				throw new Error.TypeMismatch(`Expected ${key} to be of type ${attributeType.javascriptType}, instead found type ${typeof value}.`);
			}
			return finalObject;
		}
		const returnObject = Object.keys(object).reduce((finalObject, key) => mainHandler(finalObject, key, utils.object.get(object, key)), {});
			...
			await Promise.all(Document.attributesWithSchema(returnObject).map(async (key) => {
				...
				if (value === dynamooseUndefined) {
					...
					if (!isDefaultValueUndefined) {
						const newObject = mainHandler({}, key, defaultValue);
