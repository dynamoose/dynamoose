...
function DocumentCarrier(model) {
	...
	Document.attributesWithSchema = function(document) {
		// return model.schema.attributes();
		console.log("\n\n\n\n\n");
		console.log(document);
		console.log(utils.object.keys(document));
		console.log(model.schema.attributes());
		console.log([...new Set([...utils.object.keys(document), ...model.schema.attributes()])]);

		const result = model.schema.attributes().reduce((accumulator, currentValue) => {
			accumulator.push(currentValue);

			const attributeParts = currentValue.split(".");

			attributeParts.forEach((a, index) => {
				const isLastPartNumber = !isNaN(parseInt(attributeParts[index]));

				if (isLastPartNumber) {
					const parentKey = attributeParts.slice(0, index).join(".");
					const postKey = attributeParts.slice(index + 1).join(".");
					const numberOfItems = utils.object.get(document, parentKey).length;
					console.log("isLastPartNumber", currentValue, a, index, parentKey, utils.object.get(document, parentKey), postKey, numberOfItems);
					for (let i = 1; i < numberOfItems; i++) {
						// accumulator.push(`${parentKey}.${i}`);
		...
		console.log(result);
		console.log("final result", [...new Set(result)]);
		...
		const returnObject = Object.keys(object).reduce((finalObject, key) => {
			function main(key) {
				...
				if ((existsInSchema && (typeMatches || (settings.customTypesDynamo && customValueType === expectedType))) || (settings.saveUnknown && saveUnknownAllowed)) {
					...
					// attributeTypeMap[key] = attributeType;
