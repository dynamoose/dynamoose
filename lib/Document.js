...
function DocumentCarrier(model) {
	...
	Document.objectFromSchema = async function(object, settings = {}) {
		...
		if (settings.defaults) {
			await Promise.all(model.schema.attributes().map(async (key) => {
				if ((typeof returnObject[key] === "undefined" || returnObject[key] === null)) {
					const defaultValue = await model.schema.getAttributeDefaultValue(key);
					if (defaultValue) {
	...
	Document.prototype.save = function(callback) {
		const promise = Document.Model.pendingTaskPromise().then(() => this.toDynamo({"defaults": true})).then((item) => aws.ddb().putItem({
			"Item": item,
	...
	Document.prototype.conformToSchema = async function() {
		const expectedObject = await Document.objectFromSchema(this);
