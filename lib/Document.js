...
function DocumentCarrier(model) {
	...
	Document.prototype.save = function(callback) {
		const promise = Document.Model.pendingTaskPromise().then(() => this.toDynamo({"defaults": true, "validate": true, "required": true, "enum": true, "forceDefault": true})).then((item) => aws.ddb().putItem({
			"Item": item,
