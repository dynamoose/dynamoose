...
const {internalProperties} = require("./Internal");
...
function DocumentCarrier(model) {
	...
	Document.isDynamoObject = (object, recurrsive = false) => {
		...
		function isValid(value) {
			...
			return typeof value === "object" && keys.length === 1 && Schema.attributeTypes.map((type) => type.dynamodbType).includes(key) && (nestedResult || Object.keys(value[key]).length === 0 || Schema.attributeTypes.find((a) => a.dynamodbType === key).isSet);
		}
	...
	Document.objectFromSchema = async function(object, settings = {"type": "toDynamo"}) {
		...
		const getValueTypeCheckResult = (value, key, options = {}) => {
			...
			const isValidType = [((typeDetails.customType || {}).functions || {}).isOfType, internalAttributeType.isOfType].filter((a) => Boolean(a)).some((func) => func(value, settings.type));
			console.log([((typeDetails.customType || {}).functions || {}).isOfType].filter((a) => Boolean(a)).map((func) => func(value, settings.type)));
			console.log([internalAttributeType.isOfType].filter((a) => Boolean(a)).map((func) => func(value, settings.type)));
			return {typeDetails, internalAttributeType, isValidType};
		};
		const checkTypeFunction = (item) => {
			...
			if (existsInSchema) {
				const {isValidType, internalAttributeType} = getValueTypeCheckResult(value, genericKey, {"standardKey": true});
				if (!isValidType) {
					throw new Error.TypeMismatch(`Expected ${key} to be of type ${internalAttributeType.name.toLowerCase()}, instead found type ${typeof value}.`);
				} else if (internalAttributeType.isSet) {
					validParents.push({key, "infinite": true});
				} else if (/*internalAttributeType.dynamodbType === "M" || */internalAttributeType.dynamodbType === "L") {
			...
			await Promise.all(Document.attributesWithSchema(returnObject).map(async (key) => {
				...
				if (value === dynamooseUndefined) {
					...
					if (!isDefaultValueUndefined) {
						const {isValidType, internalAttributeType} = getValueTypeCheckResult(defaultValue, key);
						if (!isValidType) {
							throw new Error.TypeMismatch(`Expected ${key} to be of type ${internalAttributeType.name.toLowerCase()}, instead found type ${typeof defaultValue}.`);
						} else {
		...
		utils.object.entries(returnObject).filter((item) => typeof item[1] === "object").forEach((item) => {
			...
			try {
				...
				const Schema = require("./Schema");
				typeDetails = Schema.attributeTypes.find((type) => type.isOfType && type.isOfType(value, settings.type, settings));
			}
