...
function DocumentCarrier(model) {
	...
	Document.objectFromSchema = async function(object, settings = {"type": "toDynamo"}) {
		const returnObject = Object.keys(object).reduce((finalObject, key) => {
			...
			const customValueType = isCustomType ? typeof model.schema.getAttributeTypeDetails(key).customType.functions[settings.type](object[key]) : null;
			const attributeType = existsInSchema ? utils.attribute_types.find((type) => type.dynamodbType === model.schema.getAttributeType(key)) : null;
			...
			if ((existsInSchema && (typeMatches || (settings.customTypesDynamo && customValueType === expectedType))) || (settings.saveUnknown && saveUnknownAllowed)) {
				finalObject[key] = attributeType && attributeType[settings.type] ? attributeType[settings.type](object[key]) : object[key];
			} else if (existsInSchema && !typeMatches && !(settings.customTypesDynamo && customValueType === expectedType)) {
				throw new Error.TypeMismatch(`Expected ${key} to be of type ${attributeType.javascriptType}, instead found type ${typeof object[key]}.`);
			}
