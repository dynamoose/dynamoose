...
function DocumentCarrier(model) {
	...
	Document.objectFromSchema = async function(object, settings = {"type": "toDynamo"}) {
		...
		function mainCheck(finalObjectPre, key, value) {
			...
			const parentExistsInSchema = model.schema.attributes().includes(parentKey);
			const existsInSchema = model.schema.attributes().includes(genericKey);
			const parentTypeDetails = parentExistsInSchema && model.schema.getAttributeTypeDetails(parentKey);
			...
			const saveUnknownAllowed = utils.dynamoose.saveunknown_attribute_allowed_check(saveUnknownSetting, key);
			if ((existsInSchema && (typeMatches || (settings.customTypesDynamo && customValueType === expectedType) || (value === dynamooseUndefined))) || (settings.saveUnknown && saveUnknownAllowed)) {
