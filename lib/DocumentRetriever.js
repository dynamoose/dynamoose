...
const documentRetrieverTypes = [
	{"name": "Scan", "type": "scan", "func": (ddb) => ddb.scan}
];
	...
	function Carrier(model) {
		...
		C.prototype[`get${documentRetrieverType.name}Request`] = function() {
			...
			Object.keys(this.settings.filters).forEach((key) => {
				...
				object[`${documentRetrieverType.name}Filter`][key] = {
		...
		C.prototype.exec = function(callback) {
			...
			const prepareForReturn = (result) => {
				...
				if (this.settings.count) {
					return {
						"count": result.Count,
						[`${documentRetrieverType.type}nedCount`]: result[`${documentRetrieverType.name}nedCount`]
					};
				}
				...
				array[`${documentRetrieverType.type}nedCount`] = result[`${documentRetrieverType.name}nedCount`];
				array[`times${documentRetrieverType.name}ned`] = timesRequested;
				return array;
			};
		...
		C.prototype.where = C.prototype.filter = function(key) {
