	...
	function Carrier(model) {
			...
			constructor(object) {
				...
				if (typeof object === "object") {
					Object.keys(object).forEach((key) => {
						...
						const type = typeof value === "object" && Object.keys(value).length > 0 ? Object.keys(value)[0] : "eq";
						...
						this.settings.filters[key] = {"type": filterType.typeName, "value": typeof value[type] !== "undefined" && value[type] !== null ? value[type] : value};
					});
				} else if (object) {
		...
		C.prototype[`get${utils.capitalize_first_letter(documentRetrieverType.type)}Request`] = async function() {
			...
			if (documentRetrieverType.type === "query") {
				...
				if (!Object.values(this.settings.filters).find((filter) => filter.queryCondition === "hash")) {
					const indexes = (await model.schema.getIndexAttributes()).map((v) => v.attribute);
					Object.keys(this.settings.filters).find((key) => {
						if (indexes.includes(key) && this.settings.filters[key].type === "EQ") {
							this.settings.filters[key].queryCondition = "hash";
							return true;
						}
					});
				}
			...
			Object.keys(this.settings.filters).forEach((key, index) => {
				...
				if (!filter.queryCondition) {
					...
					case "EXISTS":
					case "NOT_EXISTS":
						expression = `attribute_${filter.type === "NOT_EXISTS" ? "not_" : ""}exists (${keys.name})`;
						delete object.ExpressionAttributeValues[keys.value];
						break;
					case "BEGINS_WITH":
			...
			if (this.settings.index) {
				...
				const preferredIndexes = utils.array_flatten(Object.values(indexes)).filter((index) => Boolean(index.KeySchema.find((key) => key.AttributeName === object.ExpressionAttributeNames["#qha"] && key.KeyType === "HASH")));
