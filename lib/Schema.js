...
class DynamoDBType {
	...
	result() {
		const subType = this.dynamodbType instanceof DynamoDBType; // Represents underlying DynamoDB type for custom types
		...
		result.isOfType = this.jsType.func ? this.jsType.func : ((val) => {
			return [{"value": this.jsType, "type": "main"}, {"value": (subType ? type.jsType : null), "type": "underlying"}].filter((a) => Boolean(a.value)).find((jsType) => typeof jsType.value === "string" ? typeof val === jsType.value : val instanceof jsType.value);
		});
		if (type.set) {
...
const attributeTypes = utils.array_flatten(attributeTypesMain.map((a) => [a, a.set])).filter((a) => Boolean(a));
...
Schema.prototype.getAttributeTypeDetails = function(key, settings = {}) {
	...
	const returnObject = {...retrieveTypeInfo(type, isSet, key)};
	if (returnObject.customType) {
		returnObject.customType = {...returnObject.customType, "functions": returnObject.customType.functions(typeSettings)};
	}
...
module.exports.attributeTypes = {
	...
	"findTypeForValue": (...args) => attributeTypes.filter((checkType) => !checkType.customType).find((checkType) => checkType.isOfType(...args))
};
