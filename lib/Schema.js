...
Schema.prototype.getIndexes = async function() {
	return (await Promise.all(this.attributes().map(async (attribute) => ({"index": await this.getAttributeSettingValue("index", attribute), attribute})))).reduce((accumulator, currentValue) => {
		...
		if (indexValue) {
			if (indexValue === true) {
				indexValue = {
					"name": `${attributeValue}GlobalIndex`,
					"global": true,
					"project": true,
					"throughput": 1 // TODO: fix this, it shouldn't default to 1 here
				};
			}

			const dynamoIndexObject = {
				"IndexName": indexValue.name,
				"KeySchema": [{"AttributeName": attributeValue, "KeyType": "HASH"}],
				"Projection": {"ProjectionType": "KEYS_ONLY"}
			};
			if (indexValue.project) {
			...
			if (indexValue.global) {
				dynamoIndexObject.ProvisionedThroughput = {
					"ReadCapacityUnits": `${indexValue.throughput}`,
					"WriteCapacityUnits": `${indexValue.throughput}`
				};
			}
