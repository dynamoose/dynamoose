...
Schema.prototype.attributes = function() {
	const main = (object, existingKey = "") => {
		return Object.keys(object).reduce((accumulator, key) => {
			...
			if (this.getAttributeType(keyWithExisting) === "M" && object[key].schema) {
	...
	case "date":
		...
		customType = {
			"functions": {
				"toDynamo": (val) => {
					try {
						return isSet ? val.map((subVal) => subVal.getTime()) : val.getTime();
					} catch (e) {
						return null;
					}
				},
				"fromDynamo": (val) => isSet ? {"values": val.values.map((subVal) => new Date(subVal))} : new Date(val)
			}
		};
		break;
	case "object":
