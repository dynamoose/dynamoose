...
const internalCache = Symbol("internalCache");
...
Schema.prototype.attributes = function() {
	if (!this[internalCache].attributes || this[internalCache].attributes.key !== JSON.stringify(this.schemaObject)) {
		this[internalCache].attributes = {
			"key": JSON.stringify(this.schemaObject),
			"value": attributes.call(this)
		};
	}

	return this[internalCache].attributes.value;
};
...
Schema.prototype.getAttributeType = function(key, value, settings = {}) {
	try {
		...
		if (e.message === `Invalid Attribute: ${key}` && value && settings.unknownAttributeAllowed) {
			return Object.keys(require("./Document")({}).toDynamo(value, {"type": "item"}))[0];
		} else {
			throw e;
		}
	}
...
Schema.prototype.getCreateTableAttributeParams = async function(model) {
	...
	Array.prototype.concat.apply([], (await Promise.all([this.getIndexAttributes(), this.getIndexRangeKeyAttributes()]))).map((obj) => obj.attribute).forEach((index) => {
