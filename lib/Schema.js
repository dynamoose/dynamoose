...
Schema.prototype.getAttributeTypeDetails = function(key) {
	if (this[internalCache].getAttributeTypeDetails[key]) {
		return this[internalCache].getAttributeTypeDetails[key];
	}
	...
	const dynamodbType = `${returnValue}${isSet ? "S" : ""}`;
	const returnObject = {
		isSet,
		customType,
		dynamodbType,
		"internalAttributeType": utils.attribute_types.find((type) => type.dynamodbType === dynamodbType)
	};
	this[internalCache].getAttributeTypeDetails[key] = returnObject;
	return returnObject;
};
