  ...
  for(var name in obj) {
    ...
    if (name === 'timestamps') {
      ...
      if (obj[name] === true) {
        ...
        if (obj[name].createdAt && obj[name].updatedAt) {
          createdAt = obj[name].createdAt
          updatedAt = obj[name].updatedAt
        } else {
          throw new errors.SchemaError('Missing createdAt and UpdatedAt attribute. Maybe set timestamps: true?')
        }
      } else {
      ...
      if(this.attributes[createdAt]) {
        throw new errors.SchemaError('Duplicate timestamp attribute: ' + createdAt);
      }
      
      if(this.attributes[updatedAt]) {
        throw new errors.SchemaError('Duplicate timestamp attribute: ' + updatedAt);
      }
          
      this.attributes[createdAt] = Attribute.create(this, createdAt, { type: Date, default: Date.now });
      this.attributes[updatedAt] = Attribute.create(this, updatedAt, { type: Date, default: Date.now, set: function() { return Date.now() } });      
    } else {
