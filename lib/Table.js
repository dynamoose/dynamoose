  ...
  for (i = 0; i < localIndexes.length; i++) {
    ...
    for (j = 0; j < remoteIndexes.length; j++) {
      if (remoteIndexes[j].IndexName === localIndexes[i].IndexName) {
        ...
        var localIndex = Object.assign({}, {
          IndexName: localIndexes[i].IndexName,
          KeySchema: localIndexes[i].KeySchema,
          Projection: localIndexes[i].Projection,
        });

        var remoteIndex = Object.assign({}, {
          IndexName: remoteIndexes[i].IndexName,
          KeySchema: remoteIndexes[i].KeySchema,
          Projection: remoteIndexes[i].Projection,
        });
