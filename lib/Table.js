...
var trace = require('debug')('dynamoose:index');
...
Table.prototype.deleteIndex = function deleteIndex(indexName) {
  debug('deleting index %s', indexName);
...
Table.prototype.createIndex = function createIndex(attributes, indexSpec) {
  debug('creating index on %s, named %s', this.name, indexSpec.IndexName);
  var deferred = Q.defer();
  var table = this;
  table.active = false;
  var ddb = this.base.ddb();
