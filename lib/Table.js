      ...
      for(var i=0;i<localIndexes.length;i++) {
        ...
        for(var j=0;j<remoteIndexes.length;j++) {
          if(remoteIndexes[j].IndexName==localIndexes[i].IndexName) {
            ...
            if(_.isEqual(_.pick(remoteIndexes[j], 'IndexName', 'KeySchema', 'Projection', 'ProvisionedThroughput'),
                    _.pick(localIndexes[i], 'IndexName', 'KeySchema', 'Projection', 'ProvisionedThroughput'))) {
              // need to delete the other index
                ddb.UpdateTable({
