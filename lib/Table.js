...
var deleteIndex = function deleteIndex(table, ddb, tableName, indexName) {
...
var createIndex = function createIndex(table, ddb, tableName, attributes, indexSpec) {
      ...
      .then(function (data) {
          ...
          if (table.options.update) {
            for (var deleteIdx in indexes.delete) {
              deleteIndex(table, ddb, table.name, indexes.delete[deleteIdx].IndexName);
            }
            for (var createIdx in indexes.create) {
              createIndex(table, ddb, table.name, localTableReq.AttributeDefinitions, indexes.create[createIdx]);
            }
          } else {
