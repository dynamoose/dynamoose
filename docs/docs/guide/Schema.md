...
| `modelType` | string \| undefined | undefined | This setting lets you define a default "Model Type" to help a model with multiple Schemas discriminate between them.  A field with a matching field defined by `modelTypeFieldName` will choose the matched schema even if a better match appears to exist.  Without multiple schemas in a model, this does nothing.
| `modelTypeFieldName` | string | "type" | This setting lets you pick which field-name applies to the `modelType` match defined above.  Without that option (and without multiple schemas in a model), this does nothing.
```js
