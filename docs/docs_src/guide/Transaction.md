...
| type | If Dynamoose should use `get` or `write` to make the transaction call. | String | By default, if all `transactions` objects are `Get`, `get` will be run, otherwise `write` will be used. |
